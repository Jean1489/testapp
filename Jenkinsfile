            ...
            steps {
                script {
                    ...
                    if (params.FRONTEND_TAG?.trim()) {
                        imageEdits << "kustomize edit set image localhost/frontend=kind-localhost:5001/frontend:${FRONTEND_TAG}"
                    }
                    if (params.BACKEND_TAG?.trim()) {
                        imageEdits << "kustomize edit set image localhost/backend=kind-localhost:5001/backend:${BACKEND_TAG}"
                    }
